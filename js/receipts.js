...
/* 类别兜底：识别出的类别若不在当前场景的可选列表里，退到通用类别（如差旅场景认不出「器件耗材票据」） */
const RECEIPT_KIND_FALLBACK = {
  "器件耗材票据": "其他票据", "设备器材票据": "其他票据", "资料图书票据": "其他票据",
  "软件服务票据": "其他票据", "竞赛报名费": "其他票据", "专家评审费": "其他票据"
};
...
function sanitizeReceiptKind(kind, kinds, defaultKind) {
  ...
  const fb = RECEIPT_KIND_FALLBACK[kind];
  if (fb && kinds.indexOf(fb) >= 0) return fb;
...
function receiptKindColor(kind) {
  ...
  if (kind === "器件耗材票据" || kind === "设备器材票据") return "purple";
  if (kind === "资料图书票据" || kind === "软件服务票据") return "teal";
  if (kind === "竞赛报名费" || kind === "专家评审费") return "amber";
  return "gray";
}
