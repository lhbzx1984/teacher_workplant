...
function courseForm(course) {
  ...
  const overlay = formModal({
    ...
    body:
      ...
      '<span class="hint">同一门课可添加多个时段，类型分「理论 / 实验 / 实践 / 上机」</span></div>' +
...
function slotRowHTML(slot, term) {
  ...
  return '<div class="slot-row">' +
    ...
    ["理论", "实验", "实践", "上机"].map(function (t) { return '<option' + ((slot.type || "理论") === t ? " selected" : "") + ">" + t + "</option>"; }).join("") +
    "</select>" +
...
function renderCourseDetail(view, courseId) {
  ...
  if (slots.length) {
    h += '<div class="card" style="padding:10px 18px"><div class="flex">' +
      ...
      slots.map(function (s) { return badge(slotLabel(s, term), s.type === "实验" ? "purple" : (s.type === "实践" ? "teal" : (s.type === "上机" ? "amber" : "blue"))); }).join("") +
...
function attendanceForm(course, session) {
  ...
  const overlay = openModal({
    ...
    body:
      ...
      fieldHTML("课型", selectHTML("type", session ? session.type : "理论", ["理论", "实验", "实践", "上机"])) +
      "</div>" +
